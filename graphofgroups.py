...
class FPGraphOfGroups(nx.MultiDiGraph):
    ...
    def addEdge(self, u, v,  group=None, omap=None, tmap=None ,**kwargs):
        """
        ...
        if not u in self: # if u is not already in the graph add it with group isomorphic to the edge group
            self.addVertex(u, edgegroup) 
            vertgroup=self.nodes[u]['group']
            omap=homo(edgegroup,vertgroup,[vertgroup.word([i]) for i in range(1,1+len(edgegroup.gens))])
        if not v in self:
            self.addVertex(v, edgegroup)
    ...
    def changeVertexGroupbyAutomorphism(self, vert, aut):
        """
    ...
    def refineVertex(self, vert, newgog, edgeupdate):
        """
        ...
        for v in newgog:
            self.addVertex(v,newgog.localgroup(v))
        for e in newgog.edges(keys=True, data=True):
            ...
            edgedata['key']=e[2]
            self.addEdge(newgog.origin(e),newgog.terminus(e),newgog.localgroup(e),newgog.getomap(e),newgog.gettmap(e), **edgedata)
