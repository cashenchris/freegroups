...
def start_heegaard(relations):
    ...
    H.delaybeforesend,H.delayafterclose=0.001,0.001
    H.expect("HIT")
    H.send("k")
...
def is_realizable(relations, maxtime = 10, full_answer=False, ambiguous_reply=None):
    """
    ...
    try:
        ...
        if re.search("The initial presentation is realizable", raw_data)!=None:
            ans=True
        elif re.search("is not realizable",  raw_data) != None:
            ans=False
        else:
            ans=ambiguous_reply
