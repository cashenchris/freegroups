...
class FGFreeGroup(FPGroup):
    ...
    def isConjugateIntoOne(F,v,w):
        ...
        """
        v0,v1=F.cyclicReducer(v)
        w0,w1=F.cyclicReducer(w)
        x=F.word([])
        power=1
        if len(v1)==0:
            conjinto=True
            ...
            power=len(v1)//len(w1)
            sgn=1
            x=F.getConjugator(v1,w1.__pow__(sgn*power))
            if x is None:
                sgn=-1
                x=F.getConjugator(v1,w1.__pow__(sgn*power))
            if x is None:
    ...
    def isConjugateInto(F,v, *wordlist):
        """
        Decide if v is conjugate into <w> for some w in wordlist.
        """
        for i in range(len(wordlist)):
            CIO=F.isConjugateIntoOne(v,wordlist[i])
            if CIO:
                CIO['index']=i
                return CIO
        return dict([])
    ...
    def maxRoot(F,w,uptoconjugacy=False, withpower=True):
        """
    ...
    def degree(F,w):
        ...
        """
        return F.maxRoot(w,uptoconjugacy=True)[1]
...
class FGSubgroupOfFree(FGSubgroup, FGFreeGroup):
    ...
    def __init__(self, supergroup, inclusionlist=[], **kwargs):
        ...
        if inclusionlist:
            ...
            try:
                ...
                spathtov=superPathToVertex(self.graph) # dict spathtov[v]=superletters along edge path from basepoint to v through marked maxtree
    ...
    def superLettersToGraphBasis(self, wordorletters, startingvert=None):
        """
    ...
    def givenBasisToGraphBasis(self, wordorletters):
        """
        Take a word in the given basis and write it in terms of the graph basis.
        """
        return self.superLettersToGraphBasis(self.inclusion(self.word(wordorletters)))

    def graphBasisToGivenBasis(self, graphletters):
        """
    ...
    def superLettersToGivenBasis(self, wordsorletters):
        """
        ...
        """
        return self.graphBasisToGivenBasis(self.superLettersToGraphBasis(wordsorletters))
    ...
    def lifts(self,w, verbose=False):
        ...
        while verts:
            ...
            while nextvert!=thisvert:
                ...
                nextvert, nextgraphletters=followPath(theSG,theletters,nextvert,withgraphletters=True)
                assert(nextvert is not None)
                vertssofar+=1
                thisword+=nextgraphletters
            liftsInSubgroup.append(self.graphBasisToGivenBasis(thisword))
            if verbose:
    ...
    def containsSubgroup(self,G):
        """
        Decide if a subgroup G of a supergroup is contained in a subgroup self.
        """
        F=commonAncestor(self,G)
        if F is None:
            return False
        return all([self.containsWord(w) for w in [G.getInclusion(F)(G.word([i])) for i in range(1,1+G.rank)]])
        
    def equivalentSubgroup(self,G):
        """
        Decide if G is the same subgroup as self.
        """
        return self.containsSubgroup(G) and G.containsSubgroup(self)
    ...
    def restrictWord(self,w):
        ...
        if w.group is self:
            ...
            graphletters=self.superLettersToGraphBasis(w) # translate w to graph basis
            if graphletters is None:
                return None
            return self.graphBasisToGivenBasis(graphletters)
        else:  
            G=commonAncestor(self,w.group)
            wgpinG=w.group.getInclusion(G)
            winG=wgpinG(w)
            superchain=[self]
            while superchain[-1] is not G:
                superchain.append(superchain[-1].supergroup)
            return self.restrictWord(superchain[-2].restrictWord(winG)) # inner call is a simple one step restriction
        
        
    def findConjugateInSubgroup(self,w):
        """
        Return a conjugate of w in self, or None if none exists.
        """
        if w.group is self:
            return self.cyclicReduce(w)
        else:
            G=commonAncestor(self,w.group)
            winG=G.cyclicReduce(w.group.getInclusion(G)(w))
            ...
            for v in newself.graph:
                graphletters=newself.superLettersToGraphBasis(winG,v)
                if graphletters is not None: # winG makes a loop at v, found a conjugate of w in newself
                    conj=newself.word(newself.graphBasisToGivenBasis(graphletters))
                    return self.restrictWord(conj)
            else:
                return None
...
def FGSubgroupOfFreeFrom(F,wlorsg,**kwargs):
    ...
    if hasattr(wlorsg,'gens'):
        inclusion=wlorsg.getInclusion(F)
...
def changeOfMarking(theSG,marking,inversemarking,edge1,edge2):
    """
...
def cosetReps(theSG):
    ...
    """
    return superPathToVertex(theSG).values()
    
def followPath(theSG,superletters,origin, withgraphletters=False):
    """
...
def subgroupIntersection(*subgroups, **kwargs):
    """
...
def normalCore(G, verbose=False):
    """
    ...
    reps=[F.word(ll) for ll in cosetReps(G.graph)] # Coset representatives for F/G.
    conjugates=[]
    if verbose:
    ...
    core=subgroupIntersection(*conjugates, verbose=verbose)
    return core


def primitiveLiftCover(wordlist, verbose=False):
    """
...
def normalPrimitiveLiftCover(wordlist, verbose=False):
    return normalCore(primitiveLiftCover(wordlist), verbose=verbose)


def cleanCover(wordlist, verbose=False):
    """
    ...
    while wl:
        ...
        if w.letters:
            ...
            nplc=normalPrimitiveLiftCover([w], verbose=verbose)
            nplcs.append(nplc)
    if verbose:
        print "Intersecting the covers"
    cc=subgroupIntersection(*nplcs, verbose=verbose)
    return cc
