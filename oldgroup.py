...
class FGSubgroup(FGGroup):
    ...
    def __init__(self,supergroup, inclusionlist, **kwargs):
        ...
        self.inclusion=Homo(self, supergroup, inclusionlist)         
...
class Word(object):
    ...
    def __ne__(self, other):
        ...
        if G is None:
            ...
            return shortlexcompare(self.group.getInclusion(G)(self).letters,other.group.getInclusion(G)(other).letters)!=0
    ...
    def __cmp__(self, other):
        ...
        if G is None:
            ...
            return shortlexcompare(self.group.getInclusion(G)(self).letters,other.group.getInclusion(G)(other).letters)
    ...
    def __pow__(self,n):
        ...
        if n == 0:
            ...
            inverse = Word(map(lambda x: -x, self.letters[::-1]), self.group)
            for i in range(-n):
    ...
    def __call__(self,supergroup=None):
        ...
        inclusionchain=Auto(self.group) # start with the identity automorphism
...
class Homo(object):
    ...
    Group homomorphism.
    """
    def __init__(self,domain, codomain, listofimagesofgens=None): 
        # listofimagesofgens[i]=word in codomain that is image of i+1 st generator of domain
        # listofimagesofgens=None is the trivial homomorphism
        self.domain=domain
        self.codomain=codomain
        if listofimagesofgens==None:
             self.images=[[] for i in range(len(domain.gens))]
        else:
             self.images=[w.letters for w in listofimagesofgens]

    def __repr__(self):
        return str(self.domain)+" -> "+str(self.codomain)+":"+str(self.images)
    
    def __str__(self):
        return str(self.domain)+" -> "+str(self.codomain)+":\n"+"\n".join([self.domain.word([i])()+" -> "+self.codomain.word(self.images[i-1])() for i in range(1,len(self.images)+1)])
    ...
    def __call__(self,w): #evaluate the homomorphism on the word w and return a word in codomain
        ...
        while theletters:
            ...
            if nextletter<0:
                imageletters.extend([-i for i in reversed(self.images[abs(nextletter)-1])])
            elif nextletter>0:
                imageletters.extend([i for i in self.images[nextletter-1]])
            else:
                raise KeyError
        return self.codomain.word(imageletters)

        
        
        
    def alpha(self):
        print str(domain)+" -> "+str(codomain)+":\n"+"\n".join([domain.word([i]).alpha()+" -> "+codomain.word(self.images[i-1]).alpha() for i in range(1,len(self.images)+1)])



def compose(alpha,beta):
    return Homo(beta.domain,alpha.codomain,[alpha(beta(beta.domain.word([i]))) for i in range(1, len(beta.images)+1)])
...
class Endo(Homo):
    ...
    """
    def __init__(self, domain, listofimagesofgens=None):
        Homo.__init__(self, domain, domain, listofimagesofgens)
...
class Auto(Endo):
    ...
    """
    def __init__(self, domain, listofimagesofgens=None):
        if listofimagesofgens is None: # listofimagesofgens=None is the identity
             Endo.__init__(self,domain,[domain.word([i]) for i in range(1,1+len(domain.gens))])
        else:
            Endo.__init__(self, domain, listofimagesofgens)
        
    def __mul__(self,other):
        return Auto(self.domain, [self(other(other.domain.word([i]))) for i in range(1, len(other.images)+1)])

    def inverse(self):
         raise RuntimeError("inverse not implemented for arbitrary automorphisms")

    def __pow__(self,n):
        result=Auto(self.domain) # the identity automorphism
        if(n)>=0:
            for i in range(n):
                result=result * self
        else:
...
class InnerAuto(Auto):
    """
    Inner automorphism.
    """
    def __init__(self, domain, gpelement): 
        listofimagesofgens=[gpelement**(-1)*domain.word([i])*gpelement for i in range(1,1+len(domain.gens))]
        Auto.__init__(self,domain, listofimagesofgens)
...
class PDHomo(object):
    ...
    def __str__(self):
        return str(self.domain)+" -> "+str(self.codomain)+":\n"+"\n".join([self.domain.word([i])()+" -> "+self(self.domain.word([i]))() for i in self.variantGenerators()])

    def __call__(self,w): #evaluate the homomorphism on the word w and return a word in codomain
        theletters=copy.copy(w.letters)
        imageword=codomain.word([])
        while theletters:
            nextletter=theletters.pop(0)
            try:
                nextword=self.images[nextletter]
            except KeyError:
                try:
                    nextword=self.images[-nextletter]**(-1)
                except KeyError:
                    raise KeyError('The map is not defined on generator '+str(nextletter)+' of the domain.')
            imageword=imageword*nextword
        return imageword
        ...
        while theletters:
            ...
            try:
                ...
                try:
                    ...
                    nextword=self.codomain.word([]) # generators not in generatorimagedict are sent to trivial word
            imageword=imageword*nextword
        return imageword

def PDcompose(alpha,beta):
    return PDHomo(beta.domain,alpha.codomain,dict([(i,alpha(beta.images[i])) for i in beta.images.keys()]))
        ...
        while theletters:
            ...
            try:
                ...
                try:
                    ...
                    nextword=self.domain.word([nextletter]) # if neither the generator nor its inverse are in the dict then it is fixed by the automorphism
            imageword=imageword*nextword
        return imageword
...
class InnerAutomorphism(Automorphism):
    ...
    def __call__(self,w):
        return gpelement**(-1)*w*gpelement
