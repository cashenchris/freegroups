...
def simplifyWordlist(F, wl1,wl2=[],withmap=False, verbose=False): 
    """
    ...
    for i in range(len(wl2)):
        root, power=F.conjugateRoot(wl2[i], withpower=True)
    ...
    for i in range(len(wl1)):
        root, power=F.conjugateRoot(wl1[i], withpower=True)
    ...
    # if wl2=[] then these are the same
        ...
        return need
freegroup.FGFreeGroup.simplifyWordlist=simplifyWordlist

def blindSimplifyWordlist(F,wordlist):
    """
...
def areEquivalentWordlists(F, wl1,wl2):
    """
    Decide if given wordlists generate same conjugacy classes of maximal cyclic subgroups in a free group F.
    """
    return not bool(F.simplifyWordlist(wl1,wl2)) and not bool(F.simplifyWordlist(wl2,wl1))
freegroup.FGFreeGroup.areEquivalentWordlists=areEquivalentWordlists
...
class WGraph(omg.OrderedMultiGraph):
    ...
    def __init__(self, originalwordlist, autominimize=False, simplified=False, verbose=False):
        ...
        if simplified:
            ...
            simplifiedwordlist=simplifyWordlist(self.group,originalwordlist)
        if autominimize:
            if verbose:
                print "Performing peak reduction"
            self.wordlist=wreduce.WhiteheadMinimal(self.group,simplifiedwordlist,simplified=True,verbose=verbose,blind=True)['wordlist']
        else:
    ...
    def permuteEdgeOrderAtVertex(self,thisvertex,inputpermutation,lastfixed=False):
        """
    ...
    def makeEdgeOrdersConsistent(self):
        """
        ...
        for v in range(1,self.rank+1):
            ...
            self.permuteEdgeOrderAtVertex(-v,thisvertexpermutation)
...
def simplifyAndMinimizeWordlist(F,wordlist, simplified=False, minimized=False, extrawordlist=None,blind=False,cutvertsonly=False, verbose=False):
    """
    ...
    """
    if not simplified:
        simplifiedwordlist, simpmap = simplifyWordlist(F,wordlist,withmap=True)
        wordmap=[]
    ...
    if minimized:
        ...
        wm.update(wreduce.WhiteheadMinimal(F,simplifiedwordlist, extrawordlist=extrawordlist,simplified=True, blind=blind, cutvertsonly=cutvertsonly, verbose=verbose))    
    return wm
...
def wgparse(F, wgraphorwordlist, extrawordlist=None, simplifyandminimize=False, simplified=False, minimized=False, blind=False, cutvertsonly=False, verbose=False):
    ...
    try:
        ...
        try:
            inc=wgraphorwordlist[0].group.getInclusion(F)
        except KeyError:
    ...
    if simplifyandminimize:
        smwl=simplifyAndMinimizeWordlist(F,results['originalwordlist'], simplified=simplified, minimized=minimized,extrawordlist=extrawordlist, blind=blind,cutvertsonly=cutvertsonly, verbose=verbose)
        results.update(smwl)
