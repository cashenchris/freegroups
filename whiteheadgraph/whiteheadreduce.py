...
def whitehead_minimal(F,wordlist,extrawordlist=None,simplified=False,verbose=False,blind=False,stopatdisconnected=False, minimizingsequence=False,cutvertsonly=False,stopatfirstreduction=False):
    ...
    stopatfirstreduction=True to stop as soon as one reduction is found. In this case returns True if some reduction is found, or False if the wordlist is already minimal.
...
def whitehead_minimal_representative(inputword):
    """
    Take as input a word in free group.
    Return a word representing a Whitehead minimal word in the same automorphism orbit. 
    ...
    if hasattr(inputword,'isalpha'):
        return resutlword()
    elif type(inputword)==list and all(type(x)==int for x in inputword):
